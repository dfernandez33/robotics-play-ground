...
def train(agent: DDPG, manager: RobotManager, epochs: int, trajectory_length: int,
    ...
    for epoch in range(epochs):
        ...
        for step in range(trajectory_length):
            ...
            print("Action taken: {} ({})".format(action, step))
            terminal = manager.execute_action(action)
            next_state = manager.get_state()
            reward = calculate_reward(terminal, next_state)
...
def calculate_reward(terminal, state):
    if terminal:
        ...
        reward = -torch.linalg.norm(state[0, :3])
        return torch.tensor([[reward.item()]])
